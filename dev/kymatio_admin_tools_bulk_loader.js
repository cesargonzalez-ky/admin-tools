(function () {
  ...
  function renderGui(container, tools) {
    ...
    async function resolveDepts() {
      ...
      deptMap = {};
      var res = await fetch(
        'https://api-dev.kymatio.xyz/v2/admin/stakeholders/companies/' + encodeURIComponent(tools.state.companyId) + '/departments',
        { headers: apiHeaders() }
      );
    ...
    async function buildEmailMap() {
      ...
      try {
        var res = await fetch(
          'https://api-dev.kymatio.xyz/v2/admin/stakeholders/companies/' + encodeURIComponent(tools.state.companyId) + '/people?login=true&email=true',
          { headers: apiHeaders() }
        );
        var data = await res.json();
        if (!res.ok) {
    ...
    async function processRow(row, idx, dateLaunch) {
      ...
      try {
        var res = await fetch('https://api-dev.kymatio.xyz/v2/admin/stakeholders/people', {
          method: 'POST',
          headers: apiHeaders(),
        ...
        if (existingId) {
          var putRes = await fetch('https://api-dev.kymatio.xyz/v2/admin/stakeholders/people/' + encodeURIComponent(existingId), {
            method: 'PUT',
            headers: apiHeaders(),
            body: JSON.stringify(payload)
          });
