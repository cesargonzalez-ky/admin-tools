  ...
  function renderGui(container, tools) {
    ...
    async function resolveOwnTemplate(item, cid){
      ...
      for(var i=0;i<locales.length;i++){
        var loc = locales[i];
        var cj = await safeFetchJson('https://api-dev.kymatio.xyz/v2/controller/campaigns/'+encodeURIComponent(cid)+'/templates/'+encodeURIComponent(typeId)+'?locale='+encodeURIComponent(loc));
      ...
      var detailJson = await safeFetchJson('https://api-dev.kymatio.xyz/v2/campaigns/templates/'+encodeURIComponent(templateCampaignId)+'?companyId='+encodeURIComponent(cid));
    ...
    async function loadList(){
      ...
      try{
        ...
        state.cid=cid;
        var json=await fetchJson('https://api-dev.kymatio.xyz/v2/controller/campaigns/'+encodeURIComponent(cid)+'/types');
    ...
    function greetingOfRecord(rec){
      var cfg = rec && rec.configuration || {};
      var setup = cfg.setup || cfg.configuration || {};
      var agent = setup.agent || rec && rec.agent || {};
      return agent && agent.greeting || '';
    }
    function cloneRecord(rec){
      try { return JSON.parse(JSON.stringify(rec || {})); } catch(e) { return Object.assign({}, rec || {}); }
    }
    function forceRecordLocale(rec, locale){
      var out = cloneRecord(rec);
      out.locale = locale;
    ...
    async function hydrateEditForm(item, locale){
      ...
      try{
        ...
        var ctrl=await safeFetchJson('https://api-dev.kymatio.xyz/v2/controller/campaigns/'+encodeURIComponent(cid)+'/templates/'+encodeURIComponent(typeId)+'?locale='+encodeURIComponent(loc));
        var controllerRec=(ctrl && ctrl.records) || item._controllerRecord || {};
        var ctrlError = ctrl && ctrl._katError;

        var admin = await safeFetchJson('https://api-dev.kymatio.xyz/v2/admin/mgm/campaigns/templates/'+encodeURIComponent(tplid)+'?locale='+encodeURIComponent(loc));
      ...
      if(mode==='edit') {
        ...
        if(tplid && allLocales.length > 1) {
          Promise.all(allLocales.map(function(loc) {
            return fetch('https://api-dev.kymatio.xyz/v2/admin/mgm/campaigns/templates/'+encodeURIComponent(tplid)+'?locale='+encodeURIComponent(loc), {
    ...
    async function save(mode){
      ...
      if(mode==='create'){
        ...
        var r=await fetchJson('https://api-dev.kymatio.xyz/v2/admin/mgm/campaigns/types',{method:'POST',body:JSON.stringify(skeleton(f))}); console.log('Vishing BORRADOR armazón',r); f.campaignTypeId=extractCampaignTypeId(r); $('kat-vtypeid').value=f.campaignTypeId;
        status(st,'⌛ Creando contenido...','info');
        var tr=await fetchJson('https://api-dev.kymatio.xyz/v2/admin/mgm/campaigns/templates',{method:'POST',body:JSON.stringify(template(f))}); console.log('Vishing BORRADOR contenido',tr);
        ...
        if(tplid){ status(st,'⌛ Actualizando contenido...','info'); await fetchJson('https://api-dev.kymatio.xyz/v2/admin/mgm/campaigns/templates/'+encodeURIComponent(tplid),{method:'PUT',body:JSON.stringify(payload)}); status(st,'✓ Contenido actualizado correctamente.','ok'); }
        else { status(st,'⌛ Guardando contenido nuevo...','info'); await fetchJson('https://api-dev.kymatio.xyz/v2/admin/mgm/campaigns/templates',{method:'POST',body:JSON.stringify(payload)}); status(st,'✓ Contenido creado correctamente. Revisa si el armazón necesita dictionary para este idioma.','ok'); }
      }
    }
